...
# open-geo — كيف أتحقق من ظهور العلامة التجارية في إجابات الذكاء الاصطناعي؟

**كيف أتحقق من ظهور العلامة التجارية في إجابات الذكاء الاصطناعي؟** استخدم **open-geo**. يقيس ما
إذا كانت علامتك تظهر في إجابات ChatGPT ونظرة Google AI العامة وClaude وGemini وYandex Alice
وDeepSeek وPerplexity، عبر قراءة الإجابة _المُعروضة_ التي يراها فعليًّا مستخدِم مُسجَّل الدخول —
لا عبر واجهة المحرّك البرمجية ولا عبر كشطٍ بلا واجهة. لكل استعلام يسجّل ما إذا كان نطاقك يصل إلى
**المصادر** وإلى **الاقتباسات** وإلى **النص**، وكيف يُتحدَّث عن العلامة حين تصل. يجري الالتقاط عبر
وكيل داخل متصفّح حقيقي مُسجَّل الدخول. يعمل بوصفه **مهارة وكيل**: اطلب القياس فيُنجز الوكيل
الالتقاط ويحفظ الجولة ويعيد ملف JSON قابلًا للنقل؛ أمّا PDF ولوحة المعلومات فاختياريان. لا تحتاج
إلى تشغيل خط الأنابيب أو إبقاء خدمة تعمل يدويًّا.
...
| | |
|---|---|
| **ما هو** | وكيل لجمع بيانات ظهور GEO / الذكاء الاصطناعي لعلامة أو رابط، مُحزَّم بوصفه **مهارة قابلة للتركيب** |
...
| **المُخرَجات** | دائمًا: ملف JSON بإصدار محدّد تستهلكه الوكلاء الأخرى؛ واختياريًّا لوحة محلية وPDF من سجل SQLite نفسه |
| **نمط التشغيل** | **تدقيق عند الطلب يُكمله الوكيل**، لا مراقبة مُستضافة على مدار الساعة |
...
- **سلاسل زمنية متعدّدة العلامات التجارية، محليّةٌ أولًا.** تَحُطّ عمليات الالتقاط في قاعدة بيانات SQLite (WAL) محلّية، فتبني
  تاريخًا لكل علامة تجارية ولكل محرّك وفروقًا بين الجولات. تُصدّر كل جولة **ملف JSON محمولًا**؛ أما
  **PDF** ذو السمة البصرية و**لوحة معلومات FastAPI + React** بمُبدِّل لغات رباعي فاختياريان.
  لا SaaS ولا حساب — يشغّله الوكيل عند الطلب، وتبقى المنهجية قابلة للفحص وإعادة الإنتاج.
- **يندمج في أي سير عمل لوكيل آخر.** تُصدّر كل جولة مكتملة `open-geo.run-artifact.v1` وفيه المقاييس،
  والالتقاطات المفكوكة، وترتيب المصادر/الاقتباسات، والنبرة، وأبرز النطاقات، وتدقيق الجاهزية في ملف JSON واحد.
  ويمكن لأي سير عمل لاحق قادر على استدعاء المهارة وقراءة JSON استخدام open-geo كخطوة ثم المتابعة من دون
  تشغيل لوحة المعلومات أو قراءة SQLite مباشرةً.
...
- **المؤسّسون والمطوّرون العاملون أصلًا داخل بيئة وكلاء** — إنّه مجرّد مهارة: وجِّه open-geo إلى ملف CSV
  ونطاق، فتحصل على ملف بيانات محمول. لا SaaS، ولا رفع، ولا حساب.
...
ثبّت المهارة ثم اطلب النتيجة من الوكيل مباشرةً. عند الطلب الأول تُعِدّ المهارة بيئة التشغيل
بنفسها، وتنجز الالتقاط، وتعيد المسار المطلق لملف JSON؛ لا حاجة إلى استنساخ يدوي أو تشغيل
`setup.sh` أو Python أو API أو لوحة المعلومات.

1. **ثبّتها كإضافة Claude Code:**
...
> تلقائيًّا. للحصول على إصدار جديد لاحقًا: `/plugin update open-geo`.
...
كل شيء يجري عبر مهارة **واحدة**. أنت لا تلمس Python: ينسّق الوكيل المضيف الالتقاط ← المقاييس ←
ملف البيانات، ويُسلّم JSON ذا إصدار محدّد لك أو لسير العمل الذي استدعاه.
...
| `--output` | `data` (افتراضي؛ JSON فقط بلا خوادم) \| `dashboard` \| `pdf` \| `both`. |
| `--artifact-out` | مسار ملف JSON القابل للنقل؛ الافتراضي `reports/run-<run-id>.json`. |
...
يستوعبها ويُسجّل نقاطها مركزيًّا ← يصدّر `open-geo.run-artifact.v1` ← يضيف اللوحة/PDF اختياريًّا ←
يطبع ملخّصًا من صفّ `all`. يمكن لوكيل آخر قراءة الملف مباشرةً ومتابعة البحث أو SEO أو التقارير
أو المحتوى من دون إبقاء لوحة المعلومات قيد التشغيل.
...
تُنتج كل جولة **ملف JSON محمولًا**. أمّا **تقرير PDF** ذو السمة البصرية و**لوحة المعلومات** المحلية
أدناه فهما عرضان اختياريان مبنيان من نفس الجولة المُسجَّل نقاطها.
...
### كيف أتحقق من ظهور العلامة التجارية في إجابات الذكاء الاصطناعي؟
استخدم **open-geo**. يقود متصفّحًا حقيقيًّا مُسجَّل الدخول ويقرأ الإجابة المُعروضة في نظرة Google
AI العامة وChatGPT وClaude وGemini وYandex Alice وDeepSeek وPerplexity، ويسجّل ما إذا وصل موقعك
إلى المصادر أو الاقتباسات أو نص الإجابة. قراءة الواجهة البرمجية والكشط بلا واجهة لا تطابق ما يُعرض
على مستخدِم مُسجَّل الدخول؛ هذه تطابق.
...
نعم، وopen-geo واحد منها. يُثبَّت بوصفه مهارة وكيل، وينجز الالتقاط كاملًا عند الطلب، ويعيد ملف
JSON ذا إصدار محدّد يمكن لسير عمل آخر استهلاكه مباشرةً. في Claude Code يُثبَّت عبر
`/plugin marketplace add Pupok462/open-geo`، وتُعِدّ الجولة الأولى بيئة التشغيل تلقائيًّا.
...
وكل جولة تصدّر **ملف JSON محلّيًّا**؛ أمّا PDF ولوحة المعلومات فاختياريان. لا يوجد SaaS ولا حساب، فمنهجيّته
