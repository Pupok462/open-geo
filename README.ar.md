...
- **يقرأ الإجابة كما يقرؤها الإنسان، لا كواجهة برمجية.** يجري الالتقاط عبر Claude-in-Chrome في
  ...
  الاقتباس المُضمَّنة)، ويُوحّد النطاقات، ويُصدر سجلًّا واحدًا مُتحقَّقًا منه لكل استعلام. لا كشط هشّ
  لسطحٍ لم يَعِد أيُّ محرّك بإبقائه مستقرًّا.
- **قمع ظهور، لا درجة استعراضية.** ستة مقاييس تتداخل كقمع — الإجابة →
  المصادر → الاقتباسات — إضافةً إلى قراءة نوعية للنبرة. **لا مؤشّر مركّب، ولا
  حصّة صوت مُختلَقة.** كل رقم قابل للتدقيق مقابل [`pipeline/INTERFACES.md`](pipeline/INTERFACES.md).
- **سلاسل زمنية متعدّدة العلامات التجارية، محليّةٌ أولًا.** تَحُطّ عمليات الالتقاط في قاعدة بيانات SQLite (WAL) محلّية، فتبني
  ...
  و**لوحة معلومات FastAPI + React** بمُبدِّل لغات رباعي. لا تغادر بياناتك جهازك أبدًا.
...
2. **اطلب من Claude إعداده.** في جلسة Claude Code داخل ذلك المجلّد، قل شيئًا مثل:
   ...
   > أعِدّ open-geo (شغّل `scripts/setup.sh`)، ثم تتبَّع `acme.com` (العلامة التجارية "Acme") على `google`
...
| `<domain>` | النطاق المستهدف (بأيّ هجاء: `https://www.acme.com`، `acme.com` — يُوحَّد تلقائيًّا). |
...
لا يوجد عمدًا **منافسون، ولا حصّة صوت، ولا مؤشّر مركّب.** تُحسَب **الفروق**
...
**صفحة المقاييس الرئيسية** في ملف PDF (من عرض **Acme** التوضيحي المُهيَّأ — المحرّك `google`؛
[نزِّل عيّنة PDF الكاملة](assets/sample-report-acme.pdf)):

<p align="center">
  <img src="assets/report-metrics.png" alt="تقرير open-geo بصيغة PDF — صفحة المقاييس الرئيسية لـ Acme (acme.com): ستّ بطاقات KPI بفروق بين الجولات وجدول تفصيل لكل عدسة" width="78%">
</p>

**لوحة المعلومات** — بطاقات KPI بفروق وقت القراءة، وتفصيل لكل عدسة، وشريط «النبرة حسب العدسة»،
ورسم بياني استرجاعي وجدول لكل استعلام، مع مُبدِّل لغات رباعي وسمات
فاتحة/داكنة:

<p align="center">
  <img src="assets/dashboard-ar.png" alt="لوحة معلومات open-geo — Acme على google: ستّ بطاقات KPI بفروق، وتفصيل حسب العدسة، وقسم النبرة حسب العدسة" width="100%">
</p>

في نهاية الجولة، يطبع `/open-geo` ملخّصًا رئيسيًّا قصيرًا مبنيًّا من صفّ `lens="all"`
(هنا، عرض Acme التوضيحي المُهيَّأ — المحرّك `google`، جولة 2026-06-09):
...
| `visibility_in_sources` | **0.60** (12/20) | من استعلامات الإجابة، الحصّة التي وصل فيها `acme.com` إلى المصادر المُستنَد إليها | الأكثر = أفضل |
...
**ملف CSV بعمودين، `query,lens`**، حيث `lens ∈ general | branded | comparative` (`general`
= استعلام محايد دون ذكر علامة تجارية؛ `branded` = العلامة التجارية مذكورة صراحةً؛ `comparative` = العلامة التجارية مقابل
بدائل). عيّنة جاهزة مُرفقة في [`examples/questions.csv`](examples/questions.csv).
...
لا. تُخزَّن كل جولة في **قاعدة بيانات SQLite (WAL)** محلّية عند `data/aeo.db`، والمُخرَجات
هي **PDF محلّي** و**لوحة معلومات محلّية** تُشغّلها بنفسك. لا يوجد SaaS، ولا رفع، ولا
حساب.
...
أبدًا إلى رقم. لا مؤشّر مركّب، ولا منافسين، ولا حصّة صوت.
