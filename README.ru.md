...
верить, вместо того чтобы угадывать. Работает как **скилл Claude Code**: направьте `/open-geo` на
набор вопросов и домен — получите дашборд и PDF.
...
| | |
|---|---|
| **Что это** | Трекер GEO-видимости бренда или URL, упакованный как **скилл Claude Code** |
...
| **Деливераблы** | Локальный дашборд (FastAPI + React, 4 языка) и тематический PDF, который читается сам по себе — те же числа плюс таблица по запросам, список пробелов и глоссарий — из локальной истории в SQLite |
| **Модель работы** | **Точечный аудит, который вы запускаете сами**, а не круглосуточный облачный мониторинг |
| **Что нужно** | Claude Code, расширение Claude-in-Chrome, браузер, залогиненный в движок. Без API данных и без платных ключей |
| **Лицензия** | MIT |
...
**Или поставьте как плагин Claude Code** — регистрирует команду и её рабочих агентов
в любой сессии:
...
> (из клона репозитория — по-прежнему просто `/open-geo`). Плагин — обёртка для установки: сам
> пайплайн по-прежнему работает из клона (шаги 1–2 выше), и команда прямо скажет об этом, если
> вызвать её вне клона. Новый релиз подтягивается через `/plugin update open-geo`.
...
Да, open-geo им и является. Ставится как скилл Claude Code и работает одной командой
`/open-geo <questions.csv> <engine> <domain>`, управляя вашим залогиненным Chrome через расширение
Claude-in-Chrome. Можно поставить и плагином: `/plugin marketplace add Pupok462/open-geo`.
